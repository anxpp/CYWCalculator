...
public class HistoryRecordAdapter extends BaseExpandableListAdapter {
    ...
    Context context;
    //今天开始的时间
    long todayBegin = System.currentTimeMillis()/(1000*60*60*24)*(1000*60*60*24);
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        childView.valueA.setText("总资产:" + historyRecord.getA());
        childView.valueB.setText("总负债:" + historyRecord.getB());
        childView.valueN.setText("维保比例:" + historyRecord.getN());
        childView.resultX.setText("补资金:" + historyRecord.getX());
        childView.resultY.setText("卖股票:" + historyRecord.getY());
        long time = historyRecord.getTime();
        if(time>=todayBegin)
            childView.remark.setText("时间：" + new SimpleDateFormat("hh:mm:ss SSS").format(new Date(time)));
        else
